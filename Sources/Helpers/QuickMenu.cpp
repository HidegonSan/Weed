...
namespace CTRPluginFramework {
    ...
    QuickMenuItem::QuickMenuItem(const std::string &name_, const ItemType itemType_) : name(name_), itemType(itemType_) { }

    QuickMenuEntry::QuickMenuEntry(const std::string& name, VoidMethod method) : QuickMenuItem(name, ItemType::Entry) {
        methodType = MethodType::VOID;
        voidMethod = method;
        methodArg = nullptr;
    }

    QuickMenuEntry::QuickMenuEntry(const std::string& name, ArgMethod method, void* arg) : QuickMenuItem(name, ItemType::Entry) {
    ...
    QuickMenuSubMenu::QuickMenuSubMenu(const std::string& name) : QuickMenuItem(name, ItemType::SubMenu) { }

    QuickMenuSubMenu::QuickMenuSubMenu(const std::string& name, const std::vector<QuickMenuItem*>& items_) : QuickMenuItem(name, ItemType::SubMenu) {
    ...
    QuickMenu::QuickMenu(u32 hotkey) : _hotkey(hotkey, Seconds(0.5f)), _subMenuOpened(nullptr) { }
